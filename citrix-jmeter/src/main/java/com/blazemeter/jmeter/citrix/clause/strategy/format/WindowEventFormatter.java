...
public class WindowEventFormatter implements FormatStrategy {
	...
	private static String stringify(WindowEvent event) {
		...
		if (info != null) {
			value = "'" + event.getWindowState() + "' event occured on window with caption '" + info.getCaption() + "'";
		} else {
	...
	public String execute(CheckResult result, CheckResult previous, Clause clause, int index) {
		...
		if (result.isSuccessful()) {
			...
			if (clause.isUsingRegex()) {
				...
				value = stringify(event) + " whereas expecting '" + expectedState + "' on window with caption '"
