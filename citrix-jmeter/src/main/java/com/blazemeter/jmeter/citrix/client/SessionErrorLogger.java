...
public class SessionErrorLogger extends CitrixClientAdapter {
  ...
  private static KnownError lookupErrorByCode(int code) {
    return Arrays.stream(KnownError.values()).filter(e -> e.getCode() == code).findFirst()
        .orElse(null);
  }
  ...
  public void handleSessionEvent(SessionEvent sessionEvent) {
    if (sessionEvent != null) {
      ...
      if (sessionEvent.getEventType() == EventType.ERROR) {
        final int code = sessionEvent.getErrorCode();
        KnownError knownError = lookupErrorByCode(code);
        if (knownError == null) {
          LOGGER.error(CitrixUtils.getResString("session_error_logger_unknown_code_fmt", false),
              code);
        } else {
          switch (knownError) {
            case UNAVAILABLE_SESSION:
              LOGGER.error(
                  CitrixUtils.getResString("session_error_logger_unavailable_session", false));
              break;
            case NO_ERROR:
            default:
              // NOOP
        ...
        LOGGER.debug("Session Event Type {} Code {}", sessionEvent.getEventType(),
