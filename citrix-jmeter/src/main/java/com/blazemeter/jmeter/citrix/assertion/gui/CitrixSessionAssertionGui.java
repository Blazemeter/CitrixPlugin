 ...
 */
public class CitrixSessionAssertionGui
    extends AbstractAssertionGui { // NOSONAR Ignore Inheritance warning
  ...
  private Component createStatePanel() {
    ...
    panel.setBorder(BorderFactory.createTitledBorder(
        CitrixUtils.getResString("citrix_assertion_session_state_title", false))); //$NON-NLS-1$
    ButtonGroup group = new ButtonGroup();
    openState = new JRadioButton(
        CitrixUtils.getResString("citrix_assertion_session_open", false)); //$NON-NLS-1$
    closedState = new JRadioButton(
        CitrixUtils.getResString("citrix_assertion_session_closed", false)); //$NON-NLS-1$
    group.add(openState);
