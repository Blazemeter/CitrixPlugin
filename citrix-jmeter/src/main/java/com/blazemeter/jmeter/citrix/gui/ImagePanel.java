...
public class ImagePanel extends JPanel implements SelectionChangedSubject {
  ...
  public static final String HIGHLIGHT_VISIBLE_PROP_NAME = "highlightVisible";
  ...
  private final transient SelectionChangedManager selectionChangedManager =
      new SelectionChangedManager(listenerList);
  ...
  private static Color getColorFromProperty(String colorRGB) {
    try {
      ...
      return new Color(Integer.parseInt(colors[0]), Integer.parseInt(colors[1]),
  ...
  private void triggerImageHover(MouseEvent event) {
    ...
    for (ImageHoverListener listener : listeners) {
      listener.onImageHover(event);
    }
  }

  private void triggerHighlightChanged() {
    HighlightChangedListener[] listeners =
        listenerList.getListeners(HighlightChangedListener.class);
    AreaChangedEvent event =
        new AreaChangedEvent(this, highlight != null ? new Rectangle(highlight) : null);
    for (HighlightChangedListener listener : listeners) {
      listener.onHighlightChanged(event);
    }
  }
  ...
  public interface ImageHoverListener extends EventListener {
    void onImageHover(MouseEvent event);
  }

  public interface HighlightChangedListener extends EventListener {
