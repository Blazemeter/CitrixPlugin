...
public class CitrixClientFactoryHelper {
	...
	private static final String FACTORY_DEFAULT_WINDOWS = "com.blazemeter.jmeter.citrix.client.windows.WinCitrixClientFactory";
	private static final String FACTORY_DEFAULT_OTHER = "com.blazemeter.jmeter.citrix.client.mock.MockCitrixClientFactory";
	...
	public static synchronized CitrixClientFactory getInstance() throws CitrixClientFactoryException {
		if (instance == null) {
			...
			String defaultValue = SystemUtils.IS_OS_WINDOWS ? FACTORY_DEFAULT_WINDOWS : FACTORY_DEFAULT_OTHER;
			...
			try {
				...
				for (String key : properties.stringPropertyNames()) {
					if (key.startsWith(CLIENT_PROPERTIES_PFX)) {
						clientProperties.put(key.substring(CLIENT_PROPERTIES_PFX_LENGTH), properties.getProperty(key));
					}
				}
				instance.setClientProperties(clientProperties);
			} catch (Exception e) {
				String msg = MessageFormat.format("Unable to create client factory {0} from JMeter property {1}: {2}",
