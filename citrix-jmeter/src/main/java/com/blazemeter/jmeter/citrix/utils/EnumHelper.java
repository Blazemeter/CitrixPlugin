...
public class EnumHelper {
  ...
  public static <T extends Enum<T>> List<EnumWrapper<T>> getWrappers(Class<T> clazz, Set<T> filter,
    ...
    List<EnumWrapper<T>> result =
        stream.map(v -> new EnumWrapper<>(clazz, v)).collect(Collectors.toList());
    if (includeNone) {
      result.add(0, new EnumWrapper<>(clazz, null));
    }
    return result;
  }

  public static <T extends Enum<T>> List<EnumWrapper<T>> getWrappers(Class<T> clazz,
                                                                     boolean includeNone) {
    return getWrappers(clazz, null, includeNone);
  }

  public static <T extends Enum<T>> EnumWrapper<T>[] getWrappersAsArray(Class<T> clazz,
                                                                        Set<T> filter,
                                                                        boolean includeNone) {
  ...
  public static class EnumWrapper<T extends Enum<T>> {
    ...
    public String getLabel() {
      ...
      return CitrixUtils
          .getResString("enum_helper_" + clazz.getSimpleName().toLowerCase() + "_" + suffix, false);
    }
