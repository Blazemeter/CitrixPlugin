 ...
 */
public class CitrixIcaFileSaverGui extends AbstractListenerGui
    implements Clearable { // NOSONAR Ignore inheritance rule
  ...
  private Component createSavePanel() {
    filePanel = new FilePanel(CitrixUtils.getResString("citrix_file_saver_save_folder", false),
        true); // $NON-NLS-1$

    variableName =
        new JLabeledTextField(CitrixUtils.getResString("citrix_file_saver_variable", false));
    variableName.setName(CitrixIcaFileSaver.VARIABLE_NAME);

    JPanel panel = new JPanel(new GridBagLayout());
    panel.setBorder(BorderFactory.createTitledBorder(
        CitrixUtils.getResString("citrix_file_saver_format", false))); //$NON-NLS-1$
