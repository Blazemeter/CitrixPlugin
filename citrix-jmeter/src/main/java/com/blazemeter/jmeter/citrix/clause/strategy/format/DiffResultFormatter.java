...
/**
 * Provides a check result formatter dedicated to displaying change
 * detection between two screenshots.
 */
  ...
  public String execute(CheckResult result, CheckResult previous, Clause clause, int index) {
    ...
    if (previous == null) {
      ...
      if (result.isSuccessful()) {
        value = "Change detected : '" + result.getValue() + "' differs from previous '" +
            previous.getValue()
            + "'";
      } else {
        value = "No change detected : '" + result.getValue() + "' equals to previous '" +
            previous.getValue()
            + "'";
      }
    }
