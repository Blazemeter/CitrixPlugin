...
public class CitrixInstaller {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(SystemUtils.IS_OS_WINDOWS) 
        {
            ...
            if(!is32BitsJVM) {
                errorMsgs.add("Citrix Plugin must run with a JVM 32 bits, install a 32 bits JVM and restart JMeter with it");
            }
            ...
            if(citrixReceiverInstalled) {
                ...
                if(model == ARCHITECTURE.BIT_32) {
                    checkRegKey(ALLOW_SIMULATION_API_32_PATH, ALLOW_SIMULATION_API_KEY, "1", infoMsgs, errorMsgs);
                    checkRegKey(WINDOWS_32_PREFIX, VD_LOAD_UNLOAD_TIMEOUT_KEY, "1e", infoMsgs, errorMsgs);
                } else {
                    checkRegKey(ALLOW_SIMULATION_API_64_PATH, ALLOW_SIMULATION_API_KEY, "1", infoMsgs, errorMsgs);
                    checkRegKey(WINDOWS_64_PREFIX, VD_LOAD_UNLOAD_TIMEOUT_KEY, "1e", infoMsgs, errorMsgs);
                }
            }
            if(!errorMsgs.isEmpty()) {
                reportErrorToUser(errorMsgs.stream().collect(Collectors.joining("\r\n")), "Citrix Plugin Installation errors");
            }
            if(!infoMsgs.isEmpty()) {
                reportInfoToUser(infoMsgs.stream().collect(Collectors.joining("\r\n")), "Citrix Plugin Installation informations");
            }
        }
    }
    ...
    private static void updateSaveService() throws IOException {
        File self = new File(CitrixInstaller.class.getProtectionDomain().getCodeSource().getLocation().getFile());
        ...
        if (!saveServiceContent.contains("_com.blazemeter.jmeter.citrix.sampler.CitrixSampleResultConverter=collection")) {
    ...
    private static final void checkWficaRegistered(boolean is32BitsJVM, List<String> infoMsgs, List<String> errors) {
        ...
        try {
            ...
            LOGGER.warn("ICA Client creation failed with message:{}, will try to register ocx", exception.getMessage(), exception);
            try {
                if(runCommand(REGISTER_OCX, infoMsgs, errors)) {
                    ...
                    LOGGER.error("Failed registering wfica.ocx using:{}, ensure you run it as Local Administrator", REGISTER_OCX);
                    errors.add("Failed registering wfica.ocx using:"+REGISTER_OCX+", ensure you run it as Local Administrator");
                }
            } catch (IOException | InterruptedException e) {
                LOGGER.error("Failed registering wfica.ocx using:{}, ensure you run it as Local Administrator", REGISTER_OCX, e);
                errors.add("Failed registering wfica.ocx using:"+REGISTER_OCX+", ensure you run it as Local Administrator, error:" + e.getMessage());
            }
        } finally {
            if(client != null) {
                try {
    ...
    private static final boolean checkCitrixReceiverVersion(ARCHITECTURE model, List<String> infoMsgs, List<String> errors) {
        try {
            String versionPath = model == ARCHITECTURE.BIT_32 ? 
                    "Software\\Citrix\\ICA Client\\Dazzle":
                    "SOFTWARE\\Wow6432Node\\Citrix\\Dazzle";
            LOGGER.info("Checking registry key in HKLM path:{}, key:{}", versionPath, "Version");
            String value = WinRegistry.valueForKey(WinRegistry.HKEY_LOCAL_MACHINE, versionPath, "Version");
            if(StringUtils.isEmpty(value)) {
                errors.add("No Citrix Client Receiver is installed, install it to be able to use this plugin");
                return false;
            } else {
            ...
            errors.add("No Citrix Client Receiver is installed, install it to be able to use this plugin, message:"+ex.getMessage());
            return false;
        }
    }
    
    private static final boolean checkRegKey(String path, String key, String expectedValueAsHex, List<String> infoMsgs, List<String> errors) {
        try {
            ...
            if(StringUtils.isEmpty(value)) {
                return createRegKey(path, key, Integer.toString(Integer.parseInt(expectedValueAsHex,16)), infoMsgs, errors);
            } else if ( !("0x"+expectedValueAsHex).equals(value)) { 
                LOGGER.warn("Found registry key in HKLM path:{}, key:{}, value:{}, expected value should be:{}", path, key, value, expectedValueAsHex);
                errors.add("Found registry key:" +key + " in path:"+ path+" with value:" + value + ", but value is not correct, change it to:"+expectedValueAsHex);
                return false;
            } else {
                LOGGER.info("Found registry key in HKLM path:{}, key:{}, value:{}", path, key, value, expectedValueAsHex);
            ...
            errors.add("Error reading registry key:" +key + " in path:"+ path+" trying to set to:"+expectedValueAsHex+", error:"+ex.getMessage());
            return createRegKey(path, key,  Integer.toString(Integer.parseInt(expectedValueAsHex,16)), infoMsgs, errors);
        }
    }
    ...
    private static boolean createRegKey(String path, String key, String expectedValue, 
        ...
        try {
            LOGGER.info("Adding registry key in HKLM path:{}, key:{} to value:{}", path, key, expectedValue);
            // REG ADD \"%s\" /v \"%s\" /t REG_DWORD /d \"%s\"
            if(runCommand(String.format(ADD_DWORD, "HKEY_LOCAL_MACHINE\\"+path, key, expectedValue), infoMsgs, errors)) {
                infoMsgs.add("Updated registry key:" +key + " in path:"+ path+" to value:" + expectedValue);
            } else {
                infoMsgs.add("Could not update registry key:" +key + " in path:"+ path+" to value:" + expectedValue+ ", check you are running as local Administrator");
            }
            return true;
        } catch (Exception ex) {
            errors.add("Error creating/updating registry key:" +key + " in path:"+ path+" with value:" + expectedValue
