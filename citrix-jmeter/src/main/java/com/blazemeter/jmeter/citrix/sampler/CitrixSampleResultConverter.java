...
public class CitrixSampleResultConverter extends SampleResultConverter {
  ...
  private static void writeRectangle(HierarchicalStreamWriter writer, Rectangle rectangle,
                                     String prefix) {
    if (rectangle != null) {
   ...
   */
  @Override
  public boolean canConvert(Class clazz) { // superclass
    // does
    // not
    // support
    // types
  ...
  @Override
  protected void retrieveAttributes(HierarchicalStreamReader reader, UnmarshallingContext context,
                                    SampleResult res) {
    ...
    if (!StringUtils.isEmpty(checkType)) {
      ...
      boolean useRegex =
          Boolean.parseBoolean(reader.getAttribute(CLAUSE_PFX + ATT_CLAUSE_USE_REGEX));
      boolean relative =
          Boolean.parseBoolean(reader.getAttribute(CLAUSE_PFX + ATT_CLAUSE_IS_RELATIVE));
      String expectedValue = reader.getAttribute(CLAUSE_PFX + ATT_CLAUSE_EXPECTEDVALUE);
      Clause clause =
          new Clause(Enum.valueOf(CheckType.class, checkType), expectedValue, useRegex, selection);
  ...
  protected void setAttributes(HierarchicalStreamWriter writer, MarshallingContext context,
    ...
    if (clause != null) {
      ...
      writer
          .addAttribute(CLAUSE_PFX + ATT_CLAUSE_USE_REGEX, Boolean.toString(clause.isUsingRegex()));
      writer.addAttribute(CLAUSE_PFX + ATT_CLAUSE_TIMEOUT, Long.toString(clause.getTimeout()));
      writer
          .addAttribute(CLAUSE_PFX + ATT_CLAUSE_IS_RELATIVE, Boolean.toString(clause.isRelative()));
   ...
   */
  @Override
  protected boolean retrieveItem(HierarchicalStreamReader reader, UnmarshallingContext context,
                                 SampleResult res,
                                 Object subItem) {
