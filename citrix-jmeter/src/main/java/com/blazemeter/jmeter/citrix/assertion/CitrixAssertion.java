      ...
      if (snapshot != null) {
        ...
        try {
          ...
          if (!checkResult.isSuccessful()) {
            result.setResultForFailure(
                checkType.name() + " assertion: Expected value " + expectedValue
                    + " but got : " + checkResult.getValue());
          }
          if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                "{} on assertion {} gets {} result: failure={}, value='{}' expected value='{}'",
                getThreadName(), getName(), checkType, result.isFailure(), checkResult.getValue(),
                expectedValue);
          }
