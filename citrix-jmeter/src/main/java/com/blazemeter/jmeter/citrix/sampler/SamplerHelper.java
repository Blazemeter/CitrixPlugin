...
public class SamplerHelper {
	...
	private static final String MAX_KEPT_CHECKS_PROP = CitrixUtils.PROPERTIES_PFX + "clause_check_max_results";
	...
	// item informations
	private static InteractionSampler buildSamplerFromCaptureItem(CaptureItem item, boolean relative,
	...
	private static boolean isInterpretable(final InteractionEvent e) {
		...
		final boolean result = !DEACTIVATE_INTERPRETATION && (keyCode >= KeyEvent.VK_0 && keyCode <= KeyEvent.VK_9
				|| keyCode >= KeyEvent.VK_A && keyCode <= KeyEvent.VK_Z || keyCode == KeyEvent.VK_SPACE);
			...
			groups.stream().map(group -> {
				...
				try {
					...
					return buildSamplerFromCaptureItem(lastItem, capture.isRelative(), sampler -> {
						...
						LOGGER.debug(
								"Initializing sampler from last capture (keyState={}, keyCode={}) of group {} wich contains {} captures.",
						...
						case TEXT:
							...
							group.items.stream().map(c -> c.getEvent()).filter(e -> e.getKeyState() == KeyState.KEY_UP)
						...
						case SEQUENCE:
							...
							if (!group.items.isEmpty()) {
								...
								for (CaptureItem captureItem : group.items) {
									...
									KeySequenceItem item = new KeySequenceItem(event.getKeyState(), event.getKeyCode(),
	 ...
	 */
	public static List<InteractionSampler> buildMouseSamplers(Capture capture,
			Consumer<InteractionSampler> onSamplerCreated) {
		final List<InteractionSampler> samplers = new ArrayList<>();
		if (capture != null) {
			...
			groups.stream().map(group -> {
				...
				try {
					...
					return buildSamplerFromCaptureItem(lastItem, capture.isRelative(), sampler -> {
						...
						LOGGER.debug(
								"Initializing sampler from last capture (mouseAction={}) of group {} wich contains {} captures.",
		...
		if (details.isEmpty()) {
			...
			if (overflow) {
				builder.append(MessageFormat.format(
						CitrixUtils.getResString("sampler_helper_response_message_overflow", false), MAX_KEPT_CHECKS,
	...
	private static class KeyCaptureGroup {
		...
		public KeyCaptureGroup(KeySamplerType type) {
			this.type = type;
		}
	}
	...
	private static class MouseCaptureGroup {
		...
		public MouseCaptureGroup(MouseSamplerType type) {
			this.type = type;
		}
	}
}
