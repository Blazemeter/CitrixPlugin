...
public class InteractionSamplerGUI extends CitrixSamplerGUI { // NOSONAR
	...
	private static final EnumWrapper<SamplerType>[] SAMPLER_TYPES = EnumHelper.getWrappersAsArray(SamplerType.class,
			false);
	private static final EnumWrapper<KeyState>[] KEY_STATES = EnumHelper.getWrappersAsArray(KeyState.class, false);
	private static final EnumWrapper<MouseAction>[] MOUSE_ACTIONS = EnumHelper.getWrappersAsArray(MouseAction.class,
			false);
	...
	private SamplerType getCurrentSamplerType() {
		...
		final EnumWrapper<SamplerType> wrapper = (EnumWrapper<SamplerType>) cbbSamplerType.getSelectedItem();
		if (wrapper != null) {
	...
	protected void updateUI(CitrixBaseSampler sampler) {
		InteractionSampler interactionSampler = (InteractionSampler) sampler;
		cbbSamplerType.setSelectedItem(EnumHelper.lookup(SAMPLER_TYPES, interactionSampler.getSamplerType()));
	...
	protected JPanel createMainPanel() {
		...
		tbpDescription.addTab(EnumHelper.lookup(SAMPLER_TYPES, SamplerType.TEXT).getLabel(), pnlTextTab);
		tbpDescription.addTab(EnumHelper.lookup(SAMPLER_TYPES, SamplerType.KEY_SEQUENCE).getLabel(), pnlKeySequenceTab);
		tbpDescription.addTab(EnumHelper.lookup(SAMPLER_TYPES, SamplerType.MOUSE_CLICK).getLabel(), pnlMouseClickTab);
	...
	private JPanel createKeySequenceTab() {
		...
		JButton btAddItem = new JButton(CitrixUtils.getResString("interaction_sampler_add_key_sequence_item", false));
	...
	private JPanel createMouseSequenceTab() {
		...
		JButton btAddItem = new JButton(CitrixUtils.getResString("interaction_sampler_add_mouse_sequence_item", false));
		btAddItem.addActionListener(e -> {
			GuiUtils.stopTableEditing(tblMouseSequence);
		...
		JButton btRemoveSelectedItems = new JButton(
				CitrixUtils.getResString("interaction_sampler_remove_selected_mouse_sequence_items", false));
		btRemoveSelectedItems.setEnabled(false);
		btRemoveSelectedItems.addActionListener(e -> {
			GuiUtils.stopTableEditing(tblMouseSequence);
	...
	private static class MouseButtonsCellRenderer extends MouseButtonsPanel implements TableCellRenderer { // NOSONAR

		private static final long serialVersionUID = -317680945782070176L;

		public MouseButtonsCellRenderer() {
			setEditable(false);
		}
	...
	private static class ModifiersCellRenderer extends ModifiersPanel implements TableCellRenderer { // NOSONAR
	...
	private static class MouseButtonsCellEditor extends AbstractCellEditor implements TableCellEditor {

		private static final long serialVersionUID = -2637438367880728341L;

		MouseButtonsPanel pnlButtons = new MouseButtonsPanel();
	...
	private static class ModifiersCellEditor extends AbstractCellEditor implements TableCellEditor {
		...
		ModifiersPanel pnlModifiers = new ModifiersPanel();
